...
class NeopixelModeViewController: PeripheralModeViewController {
    ...
    private var neopixel: NeopixelModuleManager!
    private var components = NeopixelModuleManager.Components.fromValue(Preferences.neopixelComponents) ??  NeopixelModeViewController.kDefaultComponent
    private var ledViews = [UIView]()
    
    private var currentColor = UIColor.red
    private var colorW: Float = 0
            ...
            neopixel.start { error in
                DispatchQueue.main.async {
                    ...
                    if Preferences.neopixelIsUsingStandardBoards {
                        ...
                        board = .loadStandardBoard(boardIndex)!
                    }
                    else {
                        let stripLength = Preferences.neopixelLineBoardLength
                        board = self.createLineBoard(stripLength: stripLength)
                    }
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if let boardSelectorViewController = segue.destination as? NeopixelBoardSelectorViewController {
            ...
            boardSelectorViewController.onClickStandardBoard = { [unowned self] standardBoardIndex in
                ...
                Preferences.neopixelIsUsingStandardBoards = true
                Preferences.neopixelStandardBoardIndex = standardBoardIndex
    ...
    private func showLineStripDialog() {
        ...
        let okAction = UIAlertAction(title: localizationManager.localizedString("neopixelboardselector_linestriplength_action"), style: .default) { [weak self] _ in
            guard let self = self else { return }
            
            let stripLengthTextField = alertController.textFields![0] as UITextField
            
            if let text = stripLengthTextField.text, let stripLength = Int(text), stripLength > 0 {
                let board = self.createLineBoard(stripLength: stripLength)
                self.changeBoard(board)
                Preferences.neopixelIsUsingStandardBoards = false
                Preferences.neopixelLineBoardLength = stripLength
            }
        }
    ...
    private func createLineBoard(stripLength: Int) -> NeopixelModuleManager.Board {
        return NeopixelModuleManager.Board(name: "1x\(stripLength)", width: UInt8(stripLength), height:UInt8(1), stride: UInt8(stripLength))
    }
    
    private func updateStatusUI(isWaitingResponse: Bool) {
...
extension NeopixelModeViewController: NeopixelColorPickerViewControllerDelegate {
    ...
    private func updatePickerColorButton(isSelected: Bool) {        // Note: colorW and currentColor should have been set previously
