...
//    static let kDefaultComponentValue: UInt16 = 82       // default value: NEO_GRB + NEO_KHZ800
    ...
    enum Components {
        ...
        static var all: [Components] {
            return [ .rgb, .rbg, .grb, .gbr, .bgr, .wrgb, .wrbg, .wgrb
            , .wgbr
            , .wbrg
            , .wbgr
...
//        var type: UInt16 = kDefaultType
    ...
    func start(uartReadyCompletion:@escaping ((Error?)->())) {
        DLog("neopixel start");
    ...
    func connectNeopixel(completion: @escaping ((Bool)->())) {
    ...
    func setupNeopixel(board device: Board, components: Components, is400HzEnabled: Bool, completion: @escaping ((Bool)->())) {
        DLog("Command: Setup")
