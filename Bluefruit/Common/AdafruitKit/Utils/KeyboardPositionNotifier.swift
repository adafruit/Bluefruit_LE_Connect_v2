...
class KeyboardPositionNotifier: NSObject {
    ...
    private func registerNotifications(enabled: Bool) {
        ...
        if enabled {
            keyboardWillBeShownObserver = notificationCenter.addObserver(forName: NSNotification.Name.UIKeyboardWillShow, object: nil, queue: OperationQueue.main, using: keyboardWillBeShown)
            keyboardWillBeHiddenObserver = notificationCenter.addObserver(forName: NSNotification.Name.UIKeyboardWillHide, object: nil, queue: OperationQueue.main, using: keyboardWillBeHidden)
        }
        else {
